# vectorbt

# https://github.com/polakowo/vectorbt/blob/master/examples/PortfolioOptimization.ipynb
  
# riskfolio + vectorbt

# https://nbviewer.org/github/dcajasn/Riskfolio-Lib/blob/master/examples/Tutorial%2018.ipynb
# https://github.com/dcajasn/Riskfolio-Lib
# https://riskfolio-lib.readthedocs.io/en/latest/examples.html
# https://nbviewer.org/github/dcajasn/Riskfolio-Lib/blob/master/examples/Tutorial%201.ipynb

# Financiorencios jupyter (curva libor + ois)

# https://gist.github.com/Financioneroncios/38cef9d2f2f789b281e884e8b33f66a3
# https://gist.github.com/Financioneroncios?page=2

# Explciaciópn monetarismo y causs infalcción
# https://www.youtube.com/watch?v=857RaEq5H-E

# quantlib
# https://quantlib-python-docs.readthedocs.io/en/latest/cashflows.html#interest-rates
